...
/// <summary>
///     Recursively add/remove from awake bodies, clear joints.
/// </summary>
...
/// </summary>
    ...
    bool leavingMap)
 {
    ...
    if (enteringMap)
    {
        ...
        {
            // If there are fixtures but no physics component, attempt to ensure the body exists and then wake it. This caused a weird bug with antagonst ghost roles.
    ...
    {
        // On leaving the map, disable collisions so the entity stops immedately
        if (body != null)
        {
            var fixtures = _fixturesQuery.CompOrNull(uid);
